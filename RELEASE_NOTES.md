...
- The schema's comparer governs enum values as well as option names, so a schema built WithComparer(StringComparer.Ordinal) accepts "--type=Sales" and rejects "--type=sales". Enum values were matched case-insensitively whatever the comparer was.
- ArgumentSchemaBuilder rejects two declarations it used to accept. An option that is both required and given a default throws as it is declared, because the default could only apply when the option is left out and that is already an error. An option name or alias that does not start with one of the configured option prefixes throws at Build, because an unprefixed name is never option-shaped and could not be matched. The automatic help option is exempt from the prefix check, so "--help" still works on a schema built with WithOptionPrefixes("/").
